...
Phase 1 完了後の周辺開発として、運用監視・Google 口コミ連携・POS 連携バッチを実装。

| コンポーネント | テスト数 | 状態 |
|---|---|---|
| Phase 1 コア（T-00〜T-15） | 145 | 完了・マージ済み |
| 運用監視（タスク A） | 21 | 完了・マージ済み |
| Google 口コミ連携（タスク D） | 24 | 完了・マージ済み |
| POS 連携バッチ（タスク P） | 22 | 完了・マージ済み |
| **合計** | **211** | |
...
| **POS 連携バッチ** | pos_sync（正規化・冪等性保証）、pos_event_generator（返品→商品信頼ネガティブイベント自動生成・severity マッピング） |
...
| **周辺開発** | — | 運用監視（即時/日次/週次チェック）、Google口コミ連携（取得・AI解釈・TrustEvent生成）、POS連携バッチ（日次同期・返品イベント自動生成） | **実装完了** |
...
| データ集計 | polars |
| テスト | pytest + pytest-asyncio（211 テスト） |
...
│   └── task-pos-integration.md              ← POS連携タスクリスト（タスクP）
├── docs/
